    ...
    if (saved) {
      try {
        ...
        const parsedMapped = parsed.map((m: any) => {
          ...
          } else if (currentStats && !currentStats.includes('Mortes')) {
            if (currentStats.includes('Resets')) {
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-extrabold uppercase transition-all flex items-center gap-1.5 cursor-pointer ${
              activeView === 'sala'
                ...
                : 'bg-slate-950 text-purple-300 hover:text-white hover:bg-purple-950/60'
            }`}
          >
...
const THREAT_STYLES: Record<string, { card: string; label: string; value: string; ping: string; thumbnail?: string }> = {
  ...
  'lowers': {
    ...
    ping: 'bg-slate-400',
    thumbnail: 'https://plain-enam-prod-public.komododecks.com/202607/22/PB40ePJ8ATkoVn85YWk1/image.png'
  },
  'intermediate': {
    ...
    ping: 'bg-sky-400',
    thumbnail: 'https://static.thenounproject.com/png/2651787-200.png'
  },
  'hoher-rang': {
    ...
    ping: 'bg-red-500',
    thumbnail: 'https://plain-enam-prod-public.komododecks.com/202607/22/D8aDA9tfAUp00oVxe0W7/image.png'
  },
  'zusatzlich': {
    ...
    ping: 'bg-amber-500',
    thumbnail: 'https://plain-enam-prod-public.komododecks.com/202607/25/XzH5JjejZWx1AZJkVHtY/image.png'
  }
};
...
function RoomCard({
  ...
  const customNameText = roomData.customName || roomData.name || 'SALA DE COMANDO SOBERANA';
  ...
  const syncStatsToDiscord = async () => {
    ...
    try {
      ...
      if (statsPanelRef.current) {
        try {
          const blob = await toBlob(statsPanelRef.current, {
            cacheBust: false,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <input 
                    ...
                    placeholder="Ex: SALA DE COMANDO SOBERANA"
            ...
            {showStats && (
              ...
              >
                {isEditingStats && !isSyncing && !isSyncingStats ? (
                  ...
                  <div className="pt-2">
                    {roomData.statsText ? (
                      <div className={`grid grid-cols-2 gap-2 ${isSyncing || isSyncingStats ? '' : 'max-h-96 overflow-y-auto pr-1 custom-scrollbar'}`}>
...
function GlobalExportModal({
  ...
  const generateModalImageBlob = async (scale = statsScale, format = exportFormat) => {
    ...
    try {
      const blob = await toBlob(modalStatsPanelRef.current, {
        cacheBust: false,
        backgroundColor: '#020617',
        pixelRatio: scale,
        style: {
  ...
  const updatePreviewImage = async () => {
    ...
    try {
      const blob = await generateModalImageBlob(statsScale, exportFormat);
      if (blob) {
...
function MemberCard({ 
  ...
  const updatePreviewImage = async (scale = statsScale, format = exportFormat) => {
    setIsGeneratingPreview(true);
    try {
      const blob = await generateStatsImageBlob(scale, format);
      if (blob) {
        if (previewStatsUrl) URL.revokeObjectURL(previewStatsUrl);
        const url = URL.createObjectURL(blob);
        setPreviewStatsUrl(url);
  ...
  const handleDownloadStatsImage = async () => {
    setIsExportingStatsImage(true);
    try {
      const blob = await generateStatsImageBlob(statsScale, exportFormat);
      if (blob) {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
            ...
            <div className={`w-14 h-14 rounded-2xl overflow-hidden flex items-center justify-center text-2xl font-bold transition-all duration-300 hover:scale-110 hover:rotate-3 relative z-10 ${
              ...
              <div className={`w-full h-full flex items-center justify-center ${resolvedAvatarGlow.imgWrapper} ${
                member.customAvatarUrl ? '' : (
                  ...
                  member.name === 'Asta' ? 'bg-gradient-to-br from-indigo-500 to-purple-600' : 
                    ...
                    <button
                      ...
                      className={`w-4.5 h-4.5 rounded flex items-center justify-center text-[8px] font-black transition-all select-none active:scale-90 cursor-pointer ${
                        !member.trueResets 
                          ...
                          : 'bg-purple-950/80 border border-purple-500/40 text-purple-300 hover:bg-purple-900 hover:text-white'
                      }`}
                      ...
                      <div 
                        className={`grid gap-2 ${
                          isCapturingForImage || isSyncingStats || isExportingStatsImage 
                            ? (exportFormat === 'mobile' ? 'grid-cols-1' : 'grid-cols-2') 
                            : 'grid-cols-2 max-h-96 overflow-y-auto pr-1 custom-scrollbar'
                        }`}
...
function Section({ 
  ...
  value: string;
  onChange: (e: any) => void;
  onAdd: (skinId: string, rarityId?: string, quantity?: string, level?: string, borderStyle?: string, customBorderColor?: string, ornament?: string) => void;
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all border text-[9px] font-bold uppercase truncate hover:scale-105 active:scale-95 flex items-center justify-center gap-1.5 ${
                      selectedRarity === rarity.id 
                        ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-950 scale-100 opacity-100' 
                        : 'opacity-50'
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-all border text-[9px] font-bold uppercase truncate hover:scale-105 active:scale-95 flex items-center justify-center ${
                        selectedLevel === lvl.id
                          ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-950 scale-100 opacity-100 bg-indigo-950/40 border-indigo-500/50 text-indigo-300'
                          : 'opacity-50 bg-slate-900 border-slate-800 text-slate-400'
                      }`}
                    >
                      {lvl.name}
                    </button>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg transition-all border text-[9px] font-bold uppercase truncate hover:scale-105 active:scale-95 ${
                      selectedSkin === skin.id 
                        ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-950 scale-100' 
                        : 'opacity-70'
                    } ${skin.class}`}
                  >
                    {skin.name}
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all border text-[9px] font-bold uppercase truncate hover:scale-105 active:scale-95 flex items-center justify-center ${
                      selectedBorderStyle === st.id
                        ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-950 bg-indigo-950/40 border-indigo-500/50 text-indigo-300 font-extrabold'
                ...
                >
                  ...
                  {item.level && (
                    <span className={`text-[8px] font-extrabold px-1.5 py-0.5 rounded border font-mono tracking-wider leading-none shrink-0 whitespace-nowrap ${
                      item.level === 'MAX' || (maxLevel === 3 && item.level === '3') || (maxLevel === 4 && item.level === '4')
                        ? 'bg-amber-950/60 border-amber-500/40 text-amber-400 shadow-[0_0_8px_rgba(245,158,11,0.2)]'
                        : 'bg-slate-950/60 border-slate-800/40 text-slate-400'
                    }`}>
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg transition-all border text-[8px] font-bold uppercase truncate flex items-center justify-center gap-1.5 ${
                          editRarity === rarity.id 
                            ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-900 scale-100 opacity-100' 
                            : 'opacity-50'
                        } ${rarity.bg} ${rarity.glow}`}
                      >
                        <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: rarity.color }} />
                      ...
                      {levelOptions.map((lvl) => (
                        <button
                          ...
                          className={`p-2 rounded-lg transition-all border text-[8px] font-bold uppercase truncate flex items-center justify-center ${
                            editLevel === lvl.id
                              ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-900 scale-100 opacity-100 bg-indigo-950/40 border-indigo-500/50 text-indigo-300'
                              : 'opacity-50 bg-slate-800 border-slate-700 text-slate-400'
                          }`}
                        >
                          {lvl.name}
                        </button>
                    ...
                    {SKIN_LIST.map((skin) => (
                      <button
                        ...
                        className={`p-2 rounded-lg transition-all border text-[8px] font-bold uppercase truncate ${
                          editSkin === skin.id 
                            ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-900 scale-100' 
                            : 'opacity-70'
                        } ${skin.class}`}
                      >
                        {skin.name}
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg transition-all border text-[8px] font-bold uppercase truncate flex items-center justify-center ${
                          editBorderStyle === st.id
                            ? 'ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-slate-900 bg-indigo-950/40 border-indigo-500/50 text-indigo-300 font-extrabold'
